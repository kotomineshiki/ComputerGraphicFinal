...
int model_loading()
{
	...
	Model ourModel("D:/FreeDownloads/OpenGL_Skeletal_Animation-master/mgl_model_animation/resources/Models/nanosuit/nanosuit.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//model = glm::translate(model, glm::vec3(0.0f, -1.75f, 0.0f)); // translate it down so it's at the center of the scene
		//model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(0.1f, 0.0f, 0.0f)); 
		model = glm::scale(glm::mat4(1.0f), glm::vec3(1.0f, 1.0f, 1.0f));	// it's a bit too big for our scene, so scale it down
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	camera.ProcessMouseMovement(xoffset * 4.0f, yoffset * 4.0f );
}
