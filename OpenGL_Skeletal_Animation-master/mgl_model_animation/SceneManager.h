...
public:
	...
	SceneManager(Camera* input, float inputTime) : //rose("resources/Models/rose/rose.obj"),
	...
	*/}
	...
	void DrawWhale() {
		...
		glm::mat4 model = glm::mat4(1.0f);
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(100.0f, 600.0f, 120.0f)); // translate it down so it's at the center of the scene
		model = glm::scale(model, glm::vec3(0.2f, 0.2f, 0.2f));	// it's a bit too big for our scene, so scale it down
		//model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
